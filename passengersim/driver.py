...
class Simulation:
    ...
    def _initialize(self, config: Config):
        ...
        for pname, pvalue in config.simulation_controls:
            ...
            # These are deprecated parameters, leave them in for now to not break older config files
            elif pname in ["base_date", "dcp_hour", "dwm_lite", "double_capacity_until",
                           "show_progress_bar", "simple_k_factor", "timeframe_demand_allocation", "tot_z_factor"]:
                pass
            else:
    ...
    def run_airline_models(self, info: Any = None, departed: bool = False, debug=False):
        ...
        for airline in self.sim.airlines:
            if event_type.lower() in {"dcp"}:
                ...
                airline.rm_system.run(self.sim, airline.name, dcp_index, recording_day, event_type="dcp")
            elif event_type.lower() == "daily":
                # Daily report, this is every day prior to departure EXCEPT the specified DCPs
                airline.rm_system.run(self.sim, airline.name, dcp_index, recording_day, event_type="daily")
            elif event_type.lower() == "done":
                # Post departure processing
                airline.rm_system.run(self.sim, airline.name, dcp_index, recording_day, event_type="dcp")
                airline.rm_system.run(self.sim, airline.name, dcp_index, recording_day, event_type="departure")
                if self.sim.sample % 7 == 0:
                    # Can be used less frequently, such as ML steps on accumlated data
                    airline.rm_system.run(self.sim, airline.name, dcp_index, recording_day, event_type="weekly")
